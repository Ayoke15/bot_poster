...
- Настройка через бота (токен сообщества VK, без OAuth):
  - Кнопка **«Как подключить»** (и команда `/token_help`) — полная пошаговая инструкция Telegram + VK.
...
1) Напиши боту `/start` (или «Главная»). Подробности — кнопка **«Как подключить»** или `/token_help`.
2) В Telegram: `/set_vk <id> <токен>` (подсказка — кнопка «Формат /set_vk»).
...
Готово: `/start` → «Как подключить» / `/set_vk …` → добавить бота в канал → «Включить» (или `/enable`).
