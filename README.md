...
4) Пользователи пишут **тому же боту**, чей токен ты указал: команды `/start`, `/token_help`, `/set_vk` и т.д. обрабатываются уже твоим кодом на сервере.
...
- Настройка через бота (токен сообщества VK, без OAuth):
  - `/token_help` — пошагово, как взять токен и id группы в VK.
...
1) Напиши боту `/start`, при необходимости — `/token_help` (пошагово, где взять токен и id группы).
2) В Telegram: `/set_vk <id> <токен>`.
